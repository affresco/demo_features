...
class LogarithmicReturnFeature(AbstractFeature, ReturnsMixin):
    #
    PREFIX_LOG = "log"
    SUFFIX_RETURNS = "ret"
    ...
    def __init__(self, columns: list = None, prefix: str = None, fill_na: bool = True):
        ...
        if prefix is None:
            prefix = self.PREFIX_LOG
        super(LogarithmicReturnFeature, self).__init__(prefix=prefix)
    ...
    def __call__(self, df: pd.DataFrame):
        return self.apply(df=df, prefix=self.PREFIX_LOG, columns=self.columns)
    ...
    def apply(cls, df: pd.DataFrame, prefix: str = None, columns: list = None, spans: list = None, fill: bool = True):
        ...
        prefix = prefix or cls.PREFIX_LOG

        # Make some assertions
        cls._assert_before_applying(df=df, columns=columns, spans=spans)

        return cls.__compute_log_return(df=df, columns=columns, prefix=prefix, fill=fill)
    ...
    @classmethod
    def __compute_log_return(cls, df: pd.DataFrame, columns: list, prefix: str, fill: bool = True):
        #
        ...
        for col in columns:
            ...
            # Make computation
            res = np.log(df[col] / df[col].shift(1))
