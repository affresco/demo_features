...
class SmaFeature(AbstractFeature, AverageMixin):
    ...
    def __init__(self, spans: list = SMA_SPANS, columns: list = None, prefix: str = None,
    ...
    def apply(cls, df: pd.DataFrame, columns: list, spans: list, prefix: str = None,
        ...
        return cls.__compute_rolling_sma(df=df, columns=columns, spans=spans, prefix=prefix, ffill=ffill, bfill=bfill)

    @classmethod
    def __compute_rolling_sma(cls, df: pd.DataFrame, columns: list, spans: list, prefix: str,
                              ffill: bool = True, bfill: bool = True):
        #
